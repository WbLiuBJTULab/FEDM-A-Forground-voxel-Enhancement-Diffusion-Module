...
class DynamicVoxelVFE_ForGtMap(VFETemplate):
    ...
    def _generate_gt_voxel_maps(self, batch_dict, voxel_coords):
        ...
        # 预计算常数
        pc_range = self.point_cloud_range[:3]
        voxel_size = self.voxel_size
        grid_z, grid_y, grid_x = self.grid_size[0].item(), self.grid_size[1].item(), self.grid_size[2].item()
        ...
        for batch_idx in range(batch_size):
            ...
            # === 优化1: gt_fill_coords 局部AABB裁剪 ===
            # 对每个box，只遍历其体素AABB范围内的局部网格
            for box in valid_gt_boxes:
                # 计算8角点，转体素坐标取AABB
                l, w, h = box[3], box[4], box[5]
                x_c = torch.tensor([l/2, l/2, -l/2, -l/2, l/2, l/2, -l/2, -l/2], device=device)
                y_c = torch.tensor([w/2, -w/2, -w/2, w/2, w/2, -w/2, -w/2, w/2], device=device)
                z_c = torch.tensor([h/2, h/2, h/2, h/2, -h/2, -h/2, -h/2, -h/2], device=device)
                
                cos_h, sin_h = torch.cos(box[6]), torch.sin(box[6])
                x_rot = x_c * cos_h - y_c * sin_h + box[0]
                y_rot = x_c * sin_h + y_c * cos_h + box[1]
                z_rot = z_c + box[2]
                corners = torch.stack([x_rot, y_rot, z_rot], dim=1)
                
                corners_voxel = torch.floor((corners - pc_range) / voxel_size).long()
                z_min = max(corners_voxel[:, 2].min().item(), 0)
                z_max = min(corners_voxel[:, 2].max().item() + 1, grid_z)
                y_min = max(corners_voxel[:, 1].min().item(), 0)
                y_max = min(corners_voxel[:, 1].max().item() + 1, grid_y)
                x_min = max(corners_voxel[:, 0].min().item(), 0)
                x_max = min(corners_voxel[:, 0].max().item() + 1, grid_x)
                
                if z_min >= z_max or y_min >= y_max or x_min >= x_max:
                    continue
                
                # 生成局部网格（仅几百~几千体素，而非450万）
                z_range = torch.arange(z_min, z_max, device=device)
                y_range = torch.arange(y_min, y_max, device=device)
                x_range = torch.arange(x_min, x_max, device=device)
                zz, yy, xx = torch.meshgrid(z_range, y_range, x_range, indexing='ij')
                local_coords = torch.stack([zz.flatten(), yy.flatten(), xx.flatten()], dim=1)
                local_centers = (local_coords.float() + 0.5) * voxel_size + pc_range
                
                # 精确判断：只判断局部体素
                in_box_mask = self._points_in_boxes_3d_gpu(local_centers, box[:7].unsqueeze(0))
                inside = local_coords[in_box_mask[:, 0]]
                
                if len(inside) > 0:
                    batch_indices = torch.full((len(inside), 1), batch_idx,
                                               dtype=torch.int32, device=device)
                    gt_fill_coords_list.append(torch.cat([batch_indices, inside], dim=1))

            # === 优化2: gt_reference_coords AABB预过滤 ===
            if voxel_coords is not None and len(voxel_coords) > 0:
                ...
                if len(batch_voxel_coords) > 0:
                    ref_acc = []
                    voxel_xyz = batch_voxel_coords[:, 1:4].float()  # z, y, x
                    
                    for box in valid_gt_boxes:
                        # 快速AABB预过滤：先排除明显不在范围内的体素
                        l, w, h = box[3], box[4], box[5]
                        x_c = torch.tensor([l/2, l/2, -l/2, -l/2, l/2, l/2, -l/2, -l/2], device=device)
                        y_c = torch.tensor([w/2, -w/2, -w/2, w/2, w/2, -w/2, -w/2, w/2], device=device)
                        z_c = torch.tensor([h/2, h/2, h/2, h/2, -h/2, -h/2, -h/2, -h/2], device=device)
                        ...
                        # AABB快速剔除
                        aabb_mask = ((voxel_xyz[:, 0] >= z_min) & (voxel_xyz[:, 0] < z_max) &
                                     (voxel_xyz[:, 1] >= y_min) & (voxel_xyz[:, 1] < y_max) &
                                     (voxel_xyz[:, 2] >= x_min) & (voxel_xyz[:, 2] < x_max))
                        
                        candidates = batch_voxel_coords[aabb_mask]
                        if len(candidates) == 0:
                            continue
                        
                        # 精确判断：只对候选体素
                        cand_centers = (candidates[:, 1:4].float() + 0.5) * voxel_size + pc_range
                        in_box_mask = self._points_in_boxes_3d_gpu(cand_centers, box[:7].unsqueeze(0))
                        inside = candidates[in_box_mask[:, 0]]
                        if len(inside) > 0:
                            ref_acc.append(inside)
                    
                    if ref_acc:
                        gt_reference_coords_list.append(torch.cat(ref_acc, dim=0))
